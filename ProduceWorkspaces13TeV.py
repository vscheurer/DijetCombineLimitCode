...
masses =[1000,1200,1400,1800,2000,2500,3000,3500,4000]
for mass in masses:
  outputname = "submit_HVT_"+str(mass)+".src"
  logname = "submit_HVT_"+str(mass)+".log"
  outputfile = open(outputname,'w')
  outputfile.write('#!/bin/bash\n')
  outputfile.write("cd ${CMSSW_BASE}/src/DijetCombineLimitCode; eval `scramv1 run -sh`\n")
  outputfile.write("root -b -q 'R2JJFitter13TeV.cc("+str(mass)+","+'""'+",2,1)'\n")
  outputfile.close()
...
for mass in masses:
  ...
  outputfile.write('#!/bin/bash\n')
  outputfile.write("cd ${CMSSW_BASE}/src/DijetCombineLimitCode; eval `scramv1 run -sh`\n")
  outputfile.write("root -b -q 'R2JJFitter13TeV.cc("+str(mass)+","+'""'+",4,1)'\n")
  outputfile.close()
