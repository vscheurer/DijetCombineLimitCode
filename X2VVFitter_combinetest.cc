...
static const Int_t NCAT = 13; //for VV and qV analysis together this should be 6--> Now 21!
Double_t MMIN = 1050.;
Double_t MMAX = 4500.;
...
double scaleFactorHP=1.04;//0.99; // tau21 and jet mass scale factors MC/data
double scaleFactorLP=0.96;//1.03; // tau21 and jet mass scale factors MC/data
double scaleFactorBtag = 0.91;
...
RooArgSet* defineVariables()
{
  ...
  categories->defineType("ZZHP",0);
  categories->defineType("ZZLP",1);
  
  categories->defineType("WZHP",2);
  categories->defineType("WZLP",3);
  categories->defineType("WWHP",4);
  categories->defineType("WWLP",5);
  categories->defineType("ZZHPBtagged",6);
  categories->defineType("ZZLPBtagged",7);
  categories->defineType("WZHPBtagged",8);
  categories->defineType("WZLPBtagged",9);
  categories->defineType("WWHPBtagged",10);
  categories->defineType("WWLPBtagged",11);
  /*
...
vector<RooFitResult*> BkgModelFitBernstein(std::string altfunc,RooWorkspace* w, Bool_t dobands, std::vector<string> cat_names) {
  ...
  for (int c = ncat_min; c < ncat_min+ncat; ++c) {
   ...
   RooAbsPdf* bkg_fitTmp = new RooGenericPdf(TString::Format("bkg_fit_%s",cat_names.at(c).c_str()), "pow(1-@0, @1)/pow(@0, (@2+(@3*log(@0))))", RooArgList(*x, *p1mod, *p2mod,*p3mod)); // 4 parameter fit
...
void MakeDataCard_1Channel(RooWorkspace* w, const char* fileBaseName, const char* fileBkgName, int iChan, TString signalname, int signalsample, std::vector<string> cat_names, double mass) {
  ...
  //  HP+HP
  if( ((signalsample==0)||(signalsample==1)||(signalsample==2)||(signalsample==3)||(signalsample==4)||(signalsample==5)) && (iChan==0 ||iChan==2 ||iChan==4 ))
    scaleFactor*=(scaleFactorHP*scaleFactorLP);
  
  //  HP+LP
  if( ((signalsample==0)||(signalsample==1)||(signalsample==2)||(signalsample==3)||(signalsample==4)||(signalsample==5)) && (iChan==1||iChan==3||iChan==5))
    scaleFactor*=(scaleFactorHP*scaleFactorHP);
  
   //  HP+HP+Btag
  if( ((signalsample==5)) && (iChan==6||iChan==8 ||iChan==10) )
    scaleFactor*=(scaleFactorHP*scaleFactorLP*scaleFactorBtag);
  
  //  HP+LP=Btag
  if( ((signalsample==5)) && (iChan==7||iChan==9||iChan==11))
...
void X2VVFitter_combinetest(double mass, int signalsamples=0, int channel=0,std::string altfunc="",std::string postfix="")
{
  filePOSTfix=postfix;
  if(signalsamples==0){
