...
# ---------------------------- Create graph from data with Poisson error bars (for data) ----------------------------    
  ...
  for FunctionType in FunctionTypes:
    fitresult = doFit(FunctionType,hMassNEW,g,fFitXmin,fFitXmax,fNbins,xbins)
    M1Bkg = fitresult[3]
...
# --------------------------------------- Print results ---------------------------------------
  ...
  print "\end{table}"
  with open("f-test-output-data.txt", "a") as text_file:
...
def FitComparisons(hMassNEW,g,M1Bkg,hist_fit_residual_vsMass,FunctionType,nPar,fFitXmin,fFitXmax,fLabel,fOutputFile,chi2,dof,doSigmaBand,histoCI):
  ...
  colors = [kTeal-1,kOrange+10,kAzure+2,kViolet-2,kBlack]
  styles = [1,1,7,1,3]
  ...
  legend = TLegend(0.437351,0.6777579,0.67090289,0.9182374)
...
#   legend.AddEntry(M1Bkg[3], "5 par.   (#chi^{2}/ndof = %.2f/%i)"%(chi2[3],dof[3]),"l")
  ...
  c2.Print(cname,"cxx")
  c2.Update()
  cname = fOutputFile+"_fitComp.root"
...
if __name__ == '__main__':
  ...
  for ch in channels:
    ...
    performFit("input/DATA.root",
      "DijetMassHighPuri%s"%ch, len(massBins)-1, massBins, 1000, fitmax, "%s enriched, HP"%ch,
      "datafits/%sHP"%ch,doSigmaBand=False)
    performFit("input/DATA.root",
      "DijetMassLowPuri%s"%ch , len(massBins)-1, massBins, 1000, fitmax, "%s enriched, LP"%ch,
      "datafits/%sLP"%ch,doSigmaBand=False)
    performFit("input/DATA.root",
      "DijetMassNoPuri%s"%ch   ,len(massBins)-1, massBins, 1000, fitmax, "%s enriched, NP"%ch,
      "datafits/%sNP"%ch,doSigmaBand=False)
